...
### modified Oct 2020 after identification of sample swap between Spain and Austria
### modified Dec 2020 to test inversions & inversion breakpoints for Leave-One-Out
...
### module load intel/18.0 intelmpi/18.0 R/3.6.3; R
	...
	### load concordance object
		#load(file="/mnt/pricey_1/dropPop/concordance.Rdata")
		load(file="/scratch/aob2x/drosRTEC/dropPop/concordance.Rdata")
	...
	### merge with summary stats
		...
		load(file="/scratch/aob2x/drosRTEC/dropPop/popSS.translate.Rdata")

		### this is a major doobie. Comparative analysis of PCA with the DrosEU data clearly shows that the BA_12 and VI_12 samples are mislabeled.
		 pred.beta[pop=="BA_12", pop:="new.VI_12"]
		 pred.beta[pop=="VI_12", pop:="new.BA_12"]

		 pred.beta[,pop:=gsub("new.", "", pop)]
		...
		### save pbs objct
			...
			save(pbs, file="/scratch/aob2x/drosRTEC/dropPop/pbs.fix.Rdata")

	### load climate data
		#load("/mnt/pricey_1/dropPop/ghcnd_records.RData")
		load("/scratch/aob2x/drosRTEC/dropPop/ghcnd_records.RData")
	...
	### Calculate days above and below thresholds & include permutations. Takes a while to run so load if possible
		...
		### save weather.pop & pbs.small
			save(weather.pop, pbs.small, file="/scratch/aob2x/drosRTEC/dropPop/weather_and_pbs.fix.Rdata")
	...
	##################################################################################
		...
		### load threshold model data ( `drosRTEC_revisions/predictability_model/perm.collect.R` )



		#tlm.bi.fn <- list.files("/scratch/aob2x/drosRTEC/dropPop/newPerm", pattern="thermalLimitModels_bivariate_out_daysPrior", full.names=T)
	  ...
	  o.tlm.multi.perm <- foreach(fn.i=tlm.bi.fn[1:10])%dopar%{
	    message(fn.i)
	    #fn.i <- tlm.bi.fn[1]

		  load(fn.i)
	    o.tlm.multi[,bootSet:=as.numeric(gsub("perm", "", tstrsplit(last(tstrsplit(fn.i, "/")), "\\.")[[1]]))]
	    setnames(o.tlm.multi, "boot.num", "sub.boot.num")

	    o.tlm.multi[,modelSet:="tlm_bivariate"]

			o.tlm.multi <- o.tlm.multi[config=="m4"]
			o.tlm.multi.ag <- o.tlm.multi[,list(maxR2=max(r2)), list(sub.boot.num)]

			thresh.out.obs <- o.tlm.multi[sub.boot.num==1,
										 list(r2=r2,
											  TT=sum(r2 >= o.tlm.multi.ag[sub.boot.num!=1]$maxR2),
												n=length(r2 >= o.tlm.multi.ag[sub.boot.num!=1]$maxR2),
											  int=NA, tmax=NA, tmin=NA),
										 list(sp.th=sp.th,
											  fall.th=fall.th)]

	    return(thresh.out.obs)

	  }
	  thresh.out.obs <- rbindlist(thresh.out.obs)
		...
		setkey(o.tlm.mulfi.perm, x)
		o.tlm.mulfi.perm <- o.tlm.multi.perm[J("m4")]
	  ...
	  table(o.tlm.multi.ag.perm$x)
				...
				load(file="~/mods.perm.Rdata")

				o.tlm.multi.ag.perm[set=="Obs"][x=="m4"]
...
#	### this has been updated and is defunct. New version is here: drosRTEC_revisions/predictability_model/redoFigure3_generateData.R
...
### most of these analyses relate to older versions of the MS and are no longer relevant.
