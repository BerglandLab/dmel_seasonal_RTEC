...
grtA = read.table(args[1], header=TRUE, stringsAsFactors=FALSE)
grt = grtA[,3:ncol(grtA)]
grtL = apply(grt, MARGIN=1, FUN=function(X) (-1)*sum(log(X)) )
lesA = read.table(args[2], header=TRUE, stringsAsFactors=FALSE)
les = lesA[,3:ncol(lesA)]
