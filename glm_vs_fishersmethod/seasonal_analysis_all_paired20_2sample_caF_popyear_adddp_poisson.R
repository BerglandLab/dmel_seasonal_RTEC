...
#args = c("../../data/mel_freqdp_042016_Ne_fixed.Rdata", "mel_all_paired20_2sample_caF_popyear_dpplus10_poisson.f_s.glm", 10)
...
Nsnps = nrow(freq)
add_matrix = matrix(rpois(Nsnps*length(focal), add), ncol=length(focal))
dpfocal = dp[, focal] + add_matrix ### only looking at fall to frost
